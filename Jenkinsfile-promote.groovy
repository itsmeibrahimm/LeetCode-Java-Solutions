...
pipeline {
  ...
  stages {
    ...
    stage('Deploy to staging') {
      steps {
        ...
        script {
          common.deployHelm(env.tag, common.getServiceName(), 'staging')
        }
      }
    }
  ...
  post {
    ...
    success {
      script {
        common.notifySlackChannelDeploymentStatus("Successful Deployment", params['SHA'], "${env.BUILD_NUMBER}", "success", true)
      }
    }
    aborted {
      script {
        common.notifySlackChannelDeploymentStatus("Aborted Deployment", params['SHA'], "${env.BUILD_NUMBER}", "aborted", true)
      }
    }
    failure {
      script {
        common.notifySlackChannelDeploymentStatus("Failed Deployment", params['SHA'], "${env.BUILD_NUMBER}", "failure", true)
      }
    }
  }
}
