...
class TestMarqetaCardTransitionRepository:
    ...
    def setup(self, marqeta_card_transition_repo, marqeta_card_repo):
        self.marqeta_card_transition_repo = marqeta_card_transition_repo
        self.marqeta_card_repo = marqeta_card_repo

    async def test_create_transition(self):
        ...
        assert card
        transition: MarqetaCardTransition = await self.marqeta_card_transition_repo.create_transition(
    ...
    async def test_get_failed_transitions_success(self):
        test_token = str(uuid4())
        card: MarqetaCard = await self.marqeta_card_repo.create(
            token=test_token,
            delight_number=self.TEST_DELIGHT_NUMBER,
        ...
        new_transition_one: MarqetaCardTransition = await self.marqeta_card_transition_repo.create_transition(
            card_id=test_token, desired_state=TransitionState.ACTIVE
        )
        assert new_transition_one

        new_transition_two: MarqetaCardTransition = await self.marqeta_card_transition_repo.create_transition(
        ...
        failed_transitions: List[
            MarqetaCardTransition
        ] = await self.marqeta_card_transition_repo.get_failed_transitions(
            card_id=test_token
        )
    ...
    async def test_get_failed_transitions_failure(self):
        test_token = str(uuid4())
        transitions: List[
            MarqetaCardTransition
        ] = await self.marqeta_card_transition_repo.get_failed_transitions(
    ...
    async def test_update_transition_aborted_at_success(self):
        test_token = str(uuid4())
        card: MarqetaCard = await self.marqeta_card_repo.create(
            token=test_token,
            delight_number=self.TEST_DELIGHT_NUMBER,
        ...
        updated_transitions: List[
            MarqetaCardTransition
        ] = await self.marqeta_card_transition_repo.update_transitions_aborted_at(
    ...
    async def test_update_transition_aborted_at_failure(self):
        test_token = str(uuid4())
        card: MarqetaCard = await self.marqeta_card_repo.create(
            token=test_token,
            delight_number=self.TEST_DELIGHT_NUMBER,
    ...
    async def test_update_transition_succeeded_at_success(self):
        test_token = str(uuid4())
        card: MarqetaCard = await self.marqeta_card_repo.create(
            token=test_token,
            delight_number=self.TEST_DELIGHT_NUMBER,
        ...
        updated_transitions: List[
            MarqetaCardTransition
        ] = await self.marqeta_card_transition_repo.update_transitions_succeeded_at(
    ...
    async def test_update_transition_succeeded_at_failure(self):
        test_token = str(uuid4())
        card: MarqetaCard = await self.marqeta_card_repo.create(
            token=test_token,
            delight_number=self.TEST_DELIGHT_NUMBER,
