...
class TestGetDefaultPayoutCard:
    ...
    async def test_get_default_payout_card_without_fingerprint(
        self,
        mocker: pytest_mock.MockFixture,
        payout_card_repo: PayoutCardRepository,
        payout_method_repo: PayoutMethodRepository,
        ...
        payout_card_list.insert(len(payout_card_list) - 1, card_without_fingerprint)
        # numer of cards with fingerprint = count, and 1 card without fingerprint
        assert len(payout_card_list) == count + 1
        request = GetDefaultPayoutCardRequest(payout_account_id=payout_account.id)
