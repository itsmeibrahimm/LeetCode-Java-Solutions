...
from app.commons.config.utils import init_app_config_for_delete_payer_cron
...
logger = get_logger("delete_payer_cron")
...
app_config = init_app_config_for_delete_payer_cron()

(app_context, job_pool) = init_worker_resources(
    app_config=app_config,
    pool_name="delete_payer_cron",
    pool_size=app_config.DELETE_PAYER_CRON_JOB_POOL_DEFAULT_SIZE,
)
...
app_context.monitor.add(
    stat_resource_pool_jobs(
        stat_prefix="resource.job_pools.delete_payer",
        pool_name=job_pool.name,
        pool_job_stats=job_pool,
    ),
    interval_secs=app_config.MONITOR_INTERVAL_RESOURCE_JOB_POOL,
)
delete_payer_job = DeletePayer(
    app_context=app_context, job_pool=job_pool, statsd_client=doorstats_global
)

scheduler.add_job(
    func=delete_payer_job.run,
    name=delete_payer_job.job_name,
    trigger=app_config.DELETE_PAYER_CRON_TRIGGER,
)
...
async def handle_shutdown():
    ...
    logger.info("Cancelled delete_payer_cron pool", count=count)
    loop.stop()
